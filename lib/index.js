 ...
 */
module.exports = new XP.Class('XPPrimus', {
    ...
    leave: {
        ...
        value(channel, callback) {
            ...
            // Waterfall
            XP.waterfall([
                next => next(!XP.isString(channel, true) && XP.ValidationError('channel', 'string')), // checking
    ...
    send: {
        ...
        value(channel, event, data, callback) {
            ...
            XP.waterfall([
                next => next(!XP.isString(channel, true) && XP.ValidationError('channel', 'string')), // checking
                next => next(!XP.isString(event, true) && XP.ValidationError('event', 'string')), // checking
                next => next(!XP.isVoid(data) && !XP.isObject(data) && XP.ValidationError('data', 'Object')), // checking
    ...
    _channel(name, callback) {
        ...
        // Waterfall
        XP.waterfall([
            next => next(!XP.isString(name, true) && XP.ValidationError('name', 'string')), // checking
     ...
     */
    adapted: {
        set(val) { return this.adapted || Boolean(val); }
    },
...
// Globalizing
if (typeof window !== "undefined") { window.XPPrimus = module.exports; }
